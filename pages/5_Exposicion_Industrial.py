...
product_sidebar()
page_header(
    "Escenario de Exposición Industrial",
    "¿Qué combinaciones sector-barra merecen mayor prioridad bajo supuestos explícitos?",
)
...
with scenario_cols[2]:
    metric_card(
        "Score promedio",
        f"{filtered['avg_industrial_exposure_score'].mean():.1f}",
        "exposición media",
        kind="warning",
    )
with scenario_cols[3]:
    metric_card(
        "Score p90",
        f"{filtered['p90_industrial_exposure_score'].mean():.1f}",
        "cola del escenario",
        kind="danger",
    )
...
insight_grid(
    [
        (
            "Decision question",
            "¿Qué combinación sector-barra debe revisarse primero bajo supuestos explícitos de exposición?",
            "decision",
        ),
        (
            "Main insight",
        ...
        (
            "Recommended action",
            "Revisar cobertura contractual, participación spot, demanda mensual y sensibilidad operativa del sector seleccionado.",
            "action",
        ),
        (
            "Caveat metodológico",
            "Los escenarios son salidas de screening basadas en supuestos; se interpretan como cola de revisión, no como forecast de factura.",
            "caveat",
        ),
    ]
)
...
if filtered.empty:
    action_panel(
        "Sin resultados para el escenario",
        "Cambia sector o contrato para recuperar perfiles de exposición industrial.",
    )
else:
...
if not contract_df.empty:
    section_header(
        "Sensibilidad contractual",
        "Comparación agregada por tipo de contrato para entender cómo cambia la prioridad del escenario.",
    )
...
action_panel(
    ...
    "Una combinación con mayor score merece más atención porque reúne señal nodal, prioridad mensual, participación spot, consumo y supuestos sectoriales. Industrial exposure scenarios are assumption-based screening outputs. They should not be interpreted as invoice forecasts or project-finance valuations.",
)
