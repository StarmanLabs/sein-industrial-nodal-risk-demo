...
def priority_filter(df: pd.DataFrame, key: str = "priority") -> list[str]:
    if "due_diligence_priority_es" in df.columns:
        options = (
            df[["due_diligence_priority", "due_diligence_priority_es"]]
            .drop_duplicates()
        )
        ...
        selected_labels = st.multiselect(
            "Prioridad de due diligence",
            options["display_label"].tolist(),
            default=options["display_label"].tolist(),
            key=key,
        )
        return options[
            options["display_label"].isin(selected_labels)
        ]["due_diligence_priority"].tolist()
...
def evidence_filter(df: pd.DataFrame, key: str = "evidence") -> list[str]:
    ...
    return st.multiselect("Evidencia", options, default=options, key=key)
...
def robustness_filter(df: pd.DataFrame, key: str = "robustness") -> list[str]:
    ...
    return st.multiselect("Robustez", options, default=options, key=key)
...
def contract_selector(df: pd.DataFrame, key: str = "contract") -> str | None:
    contracts = (
        sorted(df["contract_type"].dropna().unique()) if "contract_type" in df else []
    )
    if not contracts:
        return None
